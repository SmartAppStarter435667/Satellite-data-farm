...
def get_satellite_data(coords):
    ...
    vis_params = {'bands': ['B4', 'B3', 'B2'], 'min': 0, 'max': 3000, 'dimensions': 600, 'format': 'png'}
    img_url = s2.select(['B4', 'B3', 'B2']).getThumbURL(vis_params)
    
    return {"ndvi": ndvi_val, "img_url": img_url}

def generate_ai_article(loc_name, ndvi_val):
    """AI記事生成（エラー時は標準テキストを返す）"""
    prompt = f"場所:{loc_name}、NDVI値:{ndvi_val:.2f}。この地域の植生状況と農業への影響を、専門家として解説するブログ記事をHTML形式で作成して。小見出しには<h3>タグを使って。"
    ...
    try:
        ...
        print(f"AI Generation failed for {loc_name}: {e}")
        # API制限にかかった場合の「無料・標準テンプレート」
        status = "良好" if ndvi_val > 0.4 else "注意が必要な状態"
        return f"""
        <h3>現在の観測ステータス</h3>
        <p>{loc_name}の植生指数（NDVI）は <strong>{ndvi_val:.2f}</strong> です。
        この数値から、現在は「{status}」であると推定されます。衛星データに基づき、引き続き監視を継続します。</p>
        """
...
def main():
    ...
    for loc in LOCATIONS:
        ...
        # クォータ対策済みの記事生成
        ai_article = generate_ai_article(loc['name'], data['ndvi'])
        
        content = f"""
        <div class="card">
            <h2>最新衛星画像分析</h2>
            <img src="{data['img_url']}" class="sat-img">
            <p>※Sentinel-2 リアルタイムデータ</p>
        </div>
        <div class="card">
            {ai_article}
        </div>
    ...
    # インデックスページ生成
    top_content = f"<h2>🌏 グローバル監視ポータル</h2><div class='card'><ul>{''.join(summary_list)}</ul></div>"
    generate_html("衛星データ総合案内", top_content, menu_html, "index.html")
